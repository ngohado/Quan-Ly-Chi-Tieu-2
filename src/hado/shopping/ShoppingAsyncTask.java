...
import hado.quanlychitieu.GetDataMemberDetailAsyncTask;
...
public class ShoppingAsyncTask extends AsyncTask<Invoice, Void, Boolean>{
	...
	protected Boolean doInBackground(Invoice... params) {
		...
		nameValuePair.add(new BasicNameValuePair("idmemberlq", params[0].getMemberLQ()));
	...
	protected void onPostExecute(Boolean result) {
		...
		if(result){
			...
			new GetDataMemberDetailAsyncTask(contextfa).execute();
		} else {
